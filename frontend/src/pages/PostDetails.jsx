...
const PostDetails = () => {
  ...
  const postComment = async (e, commentId) => {
    e.preventDefault();
    try {
      ...
      if (commentId) {
        ...
        // Find the updated comment and replace it in the comments state
        const updatedComments = comments.map((c) =>
          c._id === commentId ? { ...c, comment: comment } : c
        );
      ...
      setComment(""); // Clear the comment input by setting it to an empty string

      // Notify the Comment component to update
        ...
        <div className="flex flex-col mt-4">
          ...
          {comments?.map((c) => (
            <Comment
              ...
              c={c}
              post={post}
              onCommentUpdate={(commentId, editedComment) => {
                // Find and update the comment in the comments state
